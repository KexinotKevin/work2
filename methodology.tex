...
\usepackage{booktabs}
\usepackage{multirow}
\usepackage[linesnumbered, ruled, linesnumbered]{algorithm2e}
\usepackage{etoolbox}
...
\title{LGUNet: 基于图神经网络的大脑认知功能预测模型方法学介绍}
\author{方法学文档}
\date{\today}
...
\begin{abstract}
本文档详细介绍了一个基于图神经网络的大脑认知功能预测模型——LGUNet（Label-Guided UNet）及其变体LGUNet\_rela。该模型利用脑区连接网络结构，结合多关系图卷积、层级池化和注意力机制，实现对大脑认知功能的预测。文档涵盖模型总体架构、核心模块设计、训练任务、损失函数及评估指标，并给出必要的数学公式说明。
\end{abstract}
...
\begin{algorithm}
  ...
  \Input{$\mathcal{G} = (\mathbf{X}, \mathbf{A}, \mathbf{E})$：脑网络图；$K$：迭代次数；$\alpha$：阻尼系数}
  \Output{$\mathbf{PR} \in \mathbb{R}^{N \times R}$：各节点的PageRank分数矩阵}

  \BlankLine
  \tcp{初始化：均匀分布}
  $\mathbf{PR}^{(0)} \leftarrow \frac{1}{N} \cdot \mathbf{1}_{N \times R}$\;

  \tcp{对每种关系类型独立计算PageRank}
  \For{$r = 1 \textbf{ to } R$}{
    $\mathbf{e}_r \leftarrow \mathbf{E}_{:, r}$ \tcp{提取第$r$种关系的边权重}
    $(\mathbf{A}, \mathbf{norm}_r) \leftarrow \text{NormalizeEdgeWeight}(\mathbf{A}, \mathbf{e}_r)$\;

    \For{$k = 1 \textbf{ to } K$}{
      \tcp{消息传递：聚合邻居节点的PageRank分数}
      $\mathbf{PR}_{\text{new}, r} \leftarrow \texttt{AGGREGATE}(\mathbf{PR}^{(k-1)}_{:, r}, \mathbf{A}, \mathbf{norm}_r)$\;

      \tcp{阻尼更新}
      $\mathbf{PR}^{(k)}_{:, r} \leftarrow (1 - \alpha) \cdot \mathbf{PR}_{\text{new}, r} + \frac{\alpha}{N}$\;
    }
  }

  \tcp{多关系聚合}
  $\mathbf{PR}^{\text{final}} \leftarrow \frac{1}{R} \sum_{r=1}^{R} \mathbf{PR}^{(K)}_{:, r}$\;

  \Return $\mathbf{PR}^{\text{final}}$

  \tcp{辅助函数：边权重归一化}
  \Begin
    \Function{$\text{NormalizeEdgeWeight}(\mathbf{A}, \mathbf{e})$}{
      $\mathbf{d} \leftarrow \text{scatter\_add}(\mathbf{e}, \mathbf{A}[0])$ \tcp{按源节点聚合度}
      $\mathbf{d}_{\text{inv}} \leftarrow \mathbf{d}^{-1/2}$，其中 $\mathbf{d}_{\text{inv}}[d_i = \infty] = 0$\;
      $\mathbf{norm} \leftarrow \mathbf{d}_{\text{inv}}[\mathbf{A}[0]] \cdot \mathbf{e} \cdot \mathbf{d}_{\text{inv}}[\mathbf{A}[1]]$\;
...
\begin{algorithm}
  ...
  \Input{$\mathbf{x} \in \mathbb{R}^{N \times F}$：节点特征；$\mathbf{A} \in \mathbb{R}^{2 \times E}$：边索引；$\mathbf{E} \in \mathbb{R}^{E \times R}$：边属性；$\mathbf{y}$：标签向量；$p$：池化比例；$\lambda$：结构学习权重}
  \Output{$\mathbf{x}'$：池化后节点特征；$\mathbf{A}'$：更新后边索引；$\mathbf{E}'$：更新后边属性；$\text{perm}$：保留节点索引}

  \BlankLine
  \tcp{步骤1：计算节点评分}
  \Begin{
    \tcp{度评分计算}
    $\mathbf{d} \leftarrow \text{degree}(\mathbf{A}[0])$ \tcp{计算节点度}
    $\mathbf{s}^{\text{deg}} \leftarrow \sigma(\alpha \cdot \log(\mathbf{d} + \epsilon))$\;

    \tcp{PageRank评分计算（多关系平均）}
    \For{$r = 1 \textbf{ to } R$}{
      $\mathbf{e}_r \leftarrow \mathbf{E}_{:, r}$\;
      $\mathbf{PR}_r \leftarrow \text{PageRankScore}(\mathbf{A}, \mathbf{e}_r, N)$\;
    }
    $\mathbf{PR} \leftarrow \frac{1}{R} \sum_{r=1}^{R} \mathbf{PR}_r$\;
    $\mathbf{s}^{\text{pr}} \leftarrow \sigma(\beta \cdot \mathbf{PR})$\;

    \tcp{综合评分}
    $\mathbf{s} \leftarrow \frac{1}{2}(\mathbf{s}^{\text{deg}} + \mathbf{s}^{\text{pr}})$\;
  }

  \tcp{步骤2：Top-K节点选择}
  \Begin{
    $k \leftarrow \lceil p \cdot N \rceil$\;
    $\text{perm} \leftarrow \text{topk}(\mathbf{s}, k)$ \tcp{保留评分最高的$k$个节点}
    $\mathbf{x}' \leftarrow \mathbf{x}[\text{perm}] \odot \mathbf{s}[\text{perm}]$ \tcp{特征重标定}
    $(\mathbf{A}', \mathbf{E}') \leftarrow \text{filter\_adj}(\mathbf{A}, \mathbf{E}, \text{perm})$\;
  }

  \tcp{步骤3：边结构学习}
  \Begin{
    $(\mathbf{A}'', \mathbf{E}'') \leftarrow \text{add\_self\_loops}(\mathbf{A}', \mathbf{E}')$\;
    $\mathbf{A}'' \leftarrow \text{sort\_by\_source}(\mathbf{A}'')$ \tcp{按源节点排序}
    $(\mathbf{row}, \mathbf{col}) \leftarrow \mathbf{A}''$\;

    \tcp{注意力分数计算}
    $\mathbf{a}_{\text{feat}} \leftarrow [\mathbf{x}'[\mathbf{row}] ; \mathbf{x}'[\mathbf{col}]]$\;
    $\mathbf{w}_{\text{att}} \leftarrow (\mathbf{a}_{\text{feat}} \cdot \mathbf{a})^{\top}$\;
    $\tilde{\mathbf{w}}_{\text{att}} \leftarrow \text{LeakyReLU}(\mathbf{w}_{\text{att}}, \delta)$\;

    \tcp{各关系通道独立Sparsemax归一化}
    \For{$r = 1 \textbf{ to } R$}{
      $\mathbf{w}_r^{\text{fused}} \leftarrow \lambda \cdot \mathbf{E}''_{:, r} + \tilde{\mathbf{w}}_{\text{att}}$\;
      $\mathbf{E}^{\text{sparse}}_{:, r} \leftarrow \text{Sparsemax}(\mathbf{w}_r^{\text{fused}}, \mathbf{row})$\;
    }
    $\mathbf{E}' \leftarrow \mathbf{E}^{\text{sparse}}$\;
  }

  \tcp{步骤4：过滤无效边}
  \Begin{
    $\mathbf{mask} \leftarrow \mathbf{E}'_{:, 0} \neq 0$ \tcp{保留SC通道非零边}
    $\mathbf{A}' \leftarrow \mathbf{A}'[:, \mathbf{mask}]$\;
    $\mathbf{E}' \leftarrow \mathbf{E}'[\mathbf{mask}, :]$\;
  }
...
\begin{algorithm}
  ...
  \Input{$\mathbf{z} \in \mathbb{R}^n$：原始分数向量；$\mathbf{batch}$：节点批次标签}
  \Output{$\mathbf{p} \in \mathbb{R}^n$：稀疏概率分布}

  \BlankLine
  \tcp{按批次分组处理}
  \ForEach{$\text{batch\_id} \in \text{unique}(\mathbf{batch})$}{
    $\mathbf{z}_b \leftarrow \mathbf{z}[\mathbf{batch} = \text{batch\_id}]$\;
    $n_b \leftarrow |\mathbf{z}_b|$\;

    \tcp{排序并计算累积和}
    $\mathbf{z}_{\text{sorted}} \leftarrow \text{sort}(\mathbf{z}_b)$\;
    $\mathbf{cs} \leftarrow \text{cumsum}(\mathbf{z}_{\text{sorted}})$\;

    \tcp{寻找Sparsemax阈值$\tau$}
    \For{$j = 1 \textbf{ to } n_b$}{
      $\mathbf{cs}_j - j \cdot \mathbf{z}_{\text{sorted},j} \overset{?}{\leq} 1$\;
    }
    $\tau \leftarrow j^* / (j^* - 1) \cdot (\mathbf{cs}_{j^*} - 1) / j^*$\;

    \tcp{计算稀疏概率}
    $\mathbf{p}_b \leftarrow \max(\mathbf{z}_b - \tau, 0)$\;
    $\mathbf{p}[\mathbf{batch} = \text{batch\_id}] \leftarrow \mathbf{p}_b$\;
  }
...
\begin{algorithm}
  ...
  \Input{$\eta_0$：初始学习率；$T_{\text{warm}}$：预热epoch数；$P$：Patience值；$\gamma_{\text{decay}}$：衰减因子；$\eta_{\text{min}}$：最小学习率}
  \Output{$\eta_t$：第$t$个epoch的学习率}

  \BlankLine
  \tcp{初始化}
  $\ctx.best\_loss \leftarrow +\infty$\;
  $\ctx.wait\_count \leftarrow 0$\;
  $\ctx.grad\_history \leftarrow []$\;
  $\ctx.increase\_count \leftarrow 0$\;
  $\ctx.current\_lr \leftarrow \eta_0$\;

  \BlankLine
  \tcp{主循环：每个epoch执行一次}
  \For{$t = 0 \textbf{ to } T-1$}{
    \tcp{计算当前batch的平均梯度范数}
    $\bar{g}_t \leftarrow \frac{1}{B} \sum_{b=1}^{B} \|\mathbf{g}_b\|$\;
    $\ctx.grad\_history.\text{append}(\bar{g}_t)$\;

    \tcp{更新滑动窗口}
    \If{$|\ctx.grad\_history| > W$}{
      $\ctx.grad\_history.\text{pop}(0)$\;
    }

    \tcp{预热阶段：线性增长}
    \If{$t < T_{\text{warm}}$}{
      $\ctx.current\_lr \leftarrow \eta_0 \cdot (t + 1) / T_{\text{warm}}$\;
      \Return $\ctx.current\_lr$\;
    }

    \tcp{Plateau检测}
    \If{$\mathcal{L}_t < \ctx.best\_loss - \epsilon$}{
      $\ctx.best\_loss \leftarrow \mathcal{L}_t$\;
      $\ctx.wait\_count \leftarrow 0$\;
    }
    \Else{
      $\ctx.wait\_count \leftarrow \ctx.wait\_count + 1$\;
    }

    \tcp{梯度范数自适应调整}
    \If{$|\ctx.grad\_history| \geq W$}{
      $\bar{g} \leftarrow \text{mean}(\ctx.grad\_history)$\;

      \If{$\bar{g} < \tau_{\text{low}} \textbf{ and } \ctx.increase\_count < N_{\text{inc}}$}{
        $\ctx.current\_lr \leftarrow \ctx.current\_lr \times \gamma_{\text{inc}}$\;
        $\ctx.increase\_count \leftarrow \ctx.increase\_count + 1$\;
      }
      \ElseIf{$\bar{g} > \tau_{\text{high}}$}{
        $\ctx.current\_lr \leftarrow \ctx.current\_lr \times \gamma_{\text{dec}}$\;
        $\ctx.increase\_count \leftarrow 0$\;
      }
      \Else{
        $\ctx.increase\_count \leftarrow 0$\;
    ...
    \If{$\ctx.wait\_count \geq P$}{
      $\ctx.current\_lr \leftarrow \max(\ctx.current\_lr \times \gamma_{\text{decay}}, \eta_{\text{min}})$\;
      $\ctx.wait\_count \leftarrow 0$\;
      $\ctx.increase\_count \leftarrow 0$\;
    }

    \tcp{学习率边界约束}
    $\ctx.current\_lr \leftarrow \min(\ctx.current\_lr, \eta_0)$\;
...
\begin{algorithm}
  ...
  \Input{$\mathcal{L}_{\text{val}}^{(t)}$：第$t$个epoch的验证损失；$P$：Patience值；$\delta$：最小改善阈值；$T_{\text{min}}$：最小训练轮数}
  \Output{$\Theta^{\text{best}}$：最佳模型参数}

  \BlankLine
  \tcp{初始化}
  $\ctx.best\_loss \leftarrow +\infty$\;
  $\ctx.wait\_count \leftarrow 0$\;
  $\ctx.best\_state \leftarrow \texttt{None}$\;
  $\ctx.should\_stop \leftarrow \texttt{False}$\;

  \BlankLine
  \tcp{主循环：每个epoch执行一次}
  \For{$t = 0 \textbf{ to } T-1$}{
    \tcp{获取验证损失}
    $\mathcal{L}_{\text{val}} \leftarrow \mathcal{L}_{\text{val}}^{(t)}$\;
    $\ctx.model\_state \leftarrow \text{model.state\_dict()}$\;

    \tcp{检查是否改善}
    \If{$\mathcal{L}_{\text{val}} < \ctx.best\_loss - \delta$}{
      $\ctx.best\_loss \leftarrow \mathcal{L}_{\text{val}}$\;
      $\ctx.wait\_count \leftarrow 0$\;
      $\ctx.best\_state \leftarrow \ctx.model\_state$ \tcp{保存最佳状态}
    }
    \Else{
      $\ctx.wait\_count \leftarrow \ctx.wait\_count + 1$\;
    }
    ...
    \If{$t \geq T_{\text{min}} \textbf{ and } \ctx.wait\_count \geq P$}{
      $\ctx.should\_stop \leftarrow \texttt{True}$\;
      \tcp{恢复到最佳权重}
      $\text{model.load\_state\_dict}(\ctx.best\_state)$\;
...
\begin{algorithm}
  ...
  \Input{$\mathbf{x} \in \mathbb{R}^d$：输入特征向量；$\alpha$：梯度反转缩放系数}
  \Output{$\mathbf{x}' = \mathbf{x}$：前向传播输出；$-\alpha \cdot \frac{\partial \mathcal{L}}{\partial \mathbf{x}'}$：反向传播梯度}

  \BlankLine
  \tcp{前向传播：恒等映射}
  \Begin{
    $\ctx.\alpha \leftarrow \alpha$\;
    \Return $\mathbf{x}$\;
  }

  \tcp{反向传播：梯度反转}
  \Begin{
    $\mathbf{g}_{\text{forward}} \leftarrow \texttt{grad\_output}$\;
    $\mathbf{g}_{\text{backward}} \leftarrow -\ctx.\alpha \cdot \mathbf{g}_{\text{forward}}$\;
    \Return $\mathbf{g}_{\text{backward}}, \texttt{None}$\;
  }
\end{algorithm}
...
\begin{algorithm}
  ...
  \Input{$\mathcal{G} = (\mathbf{X}, \mathbf{A}, \mathbf{E})$：脑网络图数据；$\mathbf{y}_{\text{cog}}$：认知评分标签}
  \Output{$\hat{y}_{\text{cog}}$：认知预测输出；$\hat{y}_{\text{age}}$：年龄对抗输出；$\hat{y}_{\text{gender}}$：性别对抗输出}

  \BlankLine
  \tcp{特征提取阶段}
  \Begin{
    $\mathbf{x}^{(0)}, \mathbf{A}^{(0)}, \mathbf{E}^{(0)} \leftarrow \text{InputLayer}(\mathbf{X}, \mathbf{A}, \mathbf{E})$\;
    $\mathbf{x} \leftarrow \sigma(\mathbf{x}^{(0)})$\;
    $\mathbf{xs} \leftarrow [\mathbf{x}]$\;
    \tcp{编码器：层级图卷积与池化}
    \For{$l = 1 \textbf{ to } D$}{
      $\mathbf{x}, \mathbf{A}, \mathbf{E}, \text{batch} \leftarrow \text{LGMVPool}_l(\mathbf{x}, \mathbf{A}, \mathbf{E}, \mathbf{y}_{\text{cog}}, \text{batch})$\;
      $\mathbf{x}, \mathbf{A}, \mathbf{E} \leftarrow \text{relationGCN}_l(\mathbf{x}, \mathbf{A}, \mathbf{E})$\;
      $\mathbf{x} \leftarrow \text{Dropout}(\mathbf{x}, p=0.5)$\;
      $\mathbf{x} \leftarrow \sigma(\mathbf{x})$\;
      $\mathbf{xs} \leftarrow \mathbf{xs} \cup [\mathbf{x}]$\;
    }
  }

  \tcp{层级Readout：多尺度特征聚合}
  \Begin{
    $\mathbf{h}_g \leftarrow \mathbf{0}$\;
    \For{$\mathbf{x}^{(l)} \in \mathbf{xs}$}{
      $\mathbf{h}_{\text{gap}} \leftarrow \text{GAP}(\mathbf{x}^{(l)})$\;
      $\mathbf{h}_{\text{gmp}} \leftarrow \text{GMP}(\mathbf{x}^{(l)})$\;
      $\mathbf{h}_g \leftarrow \mathbf{h}_g + [\mathbf{h}_{\text{gap}} ; \mathbf{h}_{\text{gmp}}]$\;
    }
  }

  \tcp{主任务分支：认知能力预测}
  \Begin{
    $\mathbf{h}_1 \leftarrow \sigma(\mathbf{W}_1 \mathbf{h}_g + \mathbf{b}_1)$\;
    $\mathbf{h}_1 \leftarrow \text{Dropout}(\mathbf{h}_1, p)$\;
    $\mathbf{h}_2 \leftarrow \sigma(\mathbf{W}_2 \mathbf{h}_1 + \mathbf{b}_2)$\;
    $\mathbf{h}_2 \leftarrow \text{Dropout}(\mathbf{h}_2, p)$\;
    $\hat{y}_{\text{cog}} \leftarrow \mathbf{W}_3 \mathbf{h}_2 + \mathbf{b}_3$\;
  }

  \tcp{对抗分支：通过GRL反转梯度}
  \Begin{
    $\tilde{\mathbf{h}}_g \leftarrow \text{GradientReversalLayer}(\mathbf{h}_g, \alpha=1.0)$\;
    $\mathbf{h}_{\text{age},1} \leftarrow \sigma(\mathbf{W}_{\text{age},1} \tilde{\mathbf{h}}_g + \mathbf{b}_{\text{age},1})$\;
    $\hat{y}_{\text{age}} \leftarrow \mathbf{W}_{\text{age},2} \mathbf{h}_{\text{age},1} + \mathbf{b}_{\text{age},2}$\;

    $\mathbf{h}_{\text{gender},1} \leftarrow \sigma(\mathbf{W}_{\text{gender},1} \tilde{\mathbf{h}}_g + \mathbf{b}_{\text{gender},1})$\;
    $\hat{y}_{\text{gender}} \leftarrow \mathbf{W}_{\text{gender},2} \mathbf{h}_{\text{gender},1} + \mathbf{b}_{\text{gender},2}$\;
  }
...
\begin{algorithm}
  ...
  \Input{$\mathcal{D}_{\text{train}}$：训练数据集；$T$：训练轮数；$B$：批次大小}
  \Output{训练好的模型参数 $\Theta^*$}

  \BlankLine
  \For{$t = 1 \textbf{ to } T$}{
    \tcp{加载批次数据}
    $\mathcal{B} \leftarrow \text{SampleBatch}(\mathcal{D}_{\text{train}}, B)$\;
    \ForEach{$(\mathcal{G}_i, y_i^{\text{cog}}, \text{age}_i, \text{gender}_i) \in \mathcal{B}$}{
      \tcp{前向传播}
      $(\hat{y}_i^{\text{cog}}, \hat{y}_i^{\text{age}}, \hat{y}_i^{\text{gender}}) \leftarrow f_{\Theta}(\mathcal{G}_i)$\;

      \tcp{计算主任务损失}
      $\mathcal{L}_i^{\text{cog}} \leftarrow \text{SmoothL1}(y_i^{\text{cog}}, \hat{y}_i^{\text{cog}})$\;

      \tcp{计算对抗损失}
      $\mathcal{L}_i^{\text{age}} \leftarrow \text{SmoothL1}(\text{age}_i, \hat{y}_i^{\text{age}})$\;
      $\mathcal{L}_i^{\text{gender}} \leftarrow \text{BCEWithLogits}(\text{gender}_i, \hat{y}_i^{\text{gender}})$\;

      \tcp{复合损失}
      $\mathcal{L}_i \leftarrow \mathcal{L}_i^{\text{cog}} + \lambda_{\text{age}} \cdot \mathcal{L}_i^{\text{age}} + \lambda_{\text{gender}} \cdot \mathcal{L}_i^{\text{gender}}$\;
    }

    \tcp{批次平均损失}
    $\mathcal{L}_{\text{batch}} \leftarrow \frac{1}{B} \sum_{i=1}^{B} \mathcal{L}_i$\;
...
\begin{algorithm}
  ...
  \Input{$f(\cdot)$：训练好的LGUNet\_rela模型；$\mathcal{G} = (\mathbf{X}, \mathbf{A}, \mathbf{E})$：测试样本；$y$：真实标签}
  \Output{$\mathbf{S}^{\text{norm}} \in \mathbb{R}^{N \times N \times R}$：归一化显著性矩阵}

  \BlankLine
  \tcp{步骤1：启用边属性梯度计算}
  \Begin{
    $\mathbf{E}.\text{requires\_grad} \leftarrow \texttt{True}$\;
  }

  \tcp{步骤2：前向传播}
  \Begin{
    $\hat{y} \leftarrow f(\mathcal{G})$\;
  }

  \tcp{步骤3：计算显著性损失并反向传播}
  \Begin{
    $\mathcal{L}_{\text{saliency}} \leftarrow |\hat{y} - y|$ \tcp{L1损失保证梯度稳定}
    $\texttt{backward}(\mathcal{L}_{\text{saliency}})$\;
  }

  \tcp{步骤4：提取梯度}
  \Begin{
    $\mathbf{G} \leftarrow \frac{\partial \mathcal{L}_{\text{saliency}}}{\partial \mathbf{E}}$\;
  }

  \tcp{步骤5：计算Input $\times$ Gradient显著性}
  \Begin{
    \For{$r = 1 \textbf{ to } R$}{
      \ForEach{edge $k$ where $\mathbf{A}[0,k] = i, \mathbf{A}[1,k] = j$}{
        $S_{ij,r} \leftarrow |\mathbf{E}_{k,r} \cdot \mathbf{G}_{k,r}|$ \tcp{IxG计算}
      }
    }
  }

  \tcp{步骤6：通道独立Min-Max归一化}
  \Begin{
    \For{$r = 1 \textbf{ to } R$}{
      $\mathbf{S}_r \leftarrow \{ S_{ij,r} \mid (i,j) \in \mathcal{E} \}$\;
      $s_{\min} \leftarrow \min(\mathbf{S}_r)$\;
      $s_{\max} \leftarrow \max(\mathbf{S}_r)$\;
      $S_{ij,r}^{\text{norm}} \leftarrow \frac{S_{ij,r} - s_{\min}}{s_{\max} - s_{\min}}$\;
    }
  }

  \tcp{步骤7：构建对称显著性矩阵}
  \Begin{
    $\mathbf{S}^{\text{full}} \leftarrow \mathbf{0}_{N \times N \times R}$\;
    \ForEach{edge $(i,j)$}{
      \For{$r = 1 \textbf{ to } R$}{
        $\mathbf{S}^{\text{full}}_{ij,r} \leftarrow S_{ij,r}^{\text{norm}}$\;
        $\mathbf{S}^{\text{full}}_{ji,r} \leftarrow S_{ij,r}^{\text{norm}}$ \tcp{对称性约束}
      }
    }
  }
...
\begin{algorithm}
  ...
  \Input{$\{\mathbf{S}^{(m)}\}_{m=1}^{M}$：$M$个测试样本的显著性矩阵；$\tau$：百分位阈值（默认98）}
  \Output{$\bar{\mathbf{S}}$：聚合后的显著性矩阵；$\mathcal{E}_{\text{top}}$：待可视化边集}

  \BlankLine
  \tcp{步骤1：群体均值聚合}
  \Begin{
    $\bar{\mathbf{S}} \leftarrow \frac{1}{M} \sum_{m=1}^{M} \mathbf{S}^{(m)}$\;
  }

  \tcp{步骤2：阈值筛选Top边}
  \Begin{
    $\mathbf{S}_{\text{nonzero}} \leftarrow \{ \bar{S}_{ij,r} \mid \bar{S}_{ij,r} > 0 \}$\;
    $\tau_{\text{thresh}} \leftarrow P_{\tau}(\mathbf{S}_{\text{nonzero}})$ \tcp{计算百分位阈值}
  }

  \tcp{步骤3：提取高显著性边集}
  \Begin{
    $\mathcal{E}_{\text{top}} \leftarrow \{ (i,j,r) \mid \bar{S}_{ij,r} > \tau_{\text{thresh}} \}$\;
  }
