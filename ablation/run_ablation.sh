...
python run.py $BASE_ARGS --output_root ./ablation/results/ours

# 2. 移除对抗训练 (w/o GRL)
echo ">>> Running 2/4: Model w/o GRL"
python run.py $BASE_ARGS --output_root ./ablation/results/wo_grl --disable_grl

# 3. 移除 PageRank 选择 (w/o PR)
echo ">>> Running 3/4: Model w/o PageRank"
python run.py $BASE_ARGS --output_root ./ablation/results/wo_pr --disable_pr

# 4. 移除边特征学习 (w/o EL)
echo ">>> Running 4/4: Model w/o Edge Learning"
python run.py $BASE_ARGS --output_root ./ablation/results/wo_el --disable_el
