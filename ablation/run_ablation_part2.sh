...
python run.py $BASE_ARGS --cons_thresh 0.25 --output_root ./ablation/results/$NOW/thresh_0.25

echo ">>> Running A-2: Consistency Threshold = 0.50"
python run.py $BASE_ARGS --cons_thresh 0.50 --output_root ./ablation/results/$NOW/thresh_0.50

echo ">>> Running A-3: Consistency Threshold = 1.0"
python run.py $BASE_ARGS --cons_thresh 1.0 --output_root ./ablation/results/$NOW/thresh_1.0

# ----- 实验组B：Pooling Strategy 影响 -----
# (注：concat 为 Ours 默认设定，上一个脚本已跑过，这里补齐 gmp 和 gap)
echo ">>> Running B-1: Pooling Strategy = GMP only"
python run.py $BASE_ARGS --pool_strategy gmp --output_root ./ablation/results/$NOW/pool_gmp

echo ">>> Running B-2: Pooling Strategy = GAP only"
python run.py $BASE_ARGS --pool_strategy gap --output_root ./ablation/results/$NOW/pool_gap
