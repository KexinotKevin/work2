...
from layers import LGMVPool, GCN, RGCN, TwoLayerFCN
from torch_geometric.nn import global_mean_pool as gap, global_max_pool as gmp

class LGUNet(torch.nn.Module):
    def __init__(self, args, sum_res=False, act=F.relu):
        super(LGUNet, self).__init__()
        assert args.depth >= 1
        ...
        self.lin1 = torch.nn.Linear(self.in_channels*self.hidden_channels*2, self.hidden_channels)
        ...
        self.input_layer = RGCN(in_dim=self.in_channels,
        ...
        for i in range(self.depth):
            ...
            self.down_convs.append(RGCN(in_dim=self.in_channels,out_dim=self.hidden_channels,relation_num=3))

        self.fc = TwoLayerFCN(self.hidden_channels,self.hidden_channels//2)
    ...
    def forward(self, g, label_emb, batch=None):
        ...
        out = x_cl.view(x_cl.size(0)//self.in_channels, -1)
        ...
        return out, xs, edge_indices, edge_weights
        ...
        #
        # return new_x, new_edge_attr, final_x, final_edge_index, final_edge_weight, q